...
build:
	...
	go build -o polygon-edge -ldflags="\
    	-X 'github.com/LaChain/polygon-edge/versioning.Version=$(LATEST_VERSION)' \
		-X 'github.com/LaChain/polygon-edge/versioning.Commit=$(COMMIT_HASH)'\
		-X 'github.com/LaChain/polygon-edge/versioning.Branch=$(BRANCH)'\
		-X 'github.com/LaChain/polygon-edge/versioning.BuildTime=$(TIME)'" \
	main.go
